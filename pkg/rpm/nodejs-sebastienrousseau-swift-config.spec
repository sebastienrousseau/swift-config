...
%global npm_bare      swift-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-swift-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable Swift configuration providing standardized SwiftLint and SwiftFormat rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/swift-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/swift-config is a shareable configuration preset with no runtime
...
install -pm 0644 .swiftlint.yml %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 .swiftformat %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
