...
lp=lm?'/swift-config/'+lm[1]:'/swift-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/swift-config/'+lm[1]+'/search-index.json':'/swift-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
