...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/swift-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Strict SwiftLint Rules & SwiftFormat Standards](#strict-swiftlint-rules-swiftformat-standards) — before and after rule comparison
...
Install `@sebastienrousseau/swift-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/swift-config

# pnpm
pnpm add -D @sebastienrousseau/swift-config

# yarn
yarn add -D @sebastienrousseau/swift-config

# bun
bun add -d @sebastienrousseau/swift-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/swift-config.git
cd swift-config
make            # check + test
```
...
- **Target environment**: Optimized for Swift 5.10 / SwiftLint.
...
```bash
cp node_modules/@sebastienrousseau/swift-config/.swiftlint.yml ./.swiftlint.yml
cp node_modules/@sebastienrousseau/swift-config/.swiftformat ./.swiftformat
```

### Or point the tool at the installed file

```bash
swiftlint lint --config node_modules/@sebastienrousseau/swift-config/.swiftlint.yml
swiftformat --config node_modules/@sebastienrousseau/swift-config/.swiftformat .
```
...
`@sebastienrousseau/swift-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/swift-config` solves this with three deliberate engineering choices:
...
## Strict SwiftLint Rules & SwiftFormat Standards

Enforces strict concurrency checking, memory leak prevention (unowned/weak self), and deterministic Swift formatting.
...
### After (@sebastienrousseau/swift-config Enforced)
...
When maintaining legacy Swift 3/4 codebases with obsolete syntax and deprecated Foundation APIs.
